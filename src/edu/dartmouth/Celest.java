...
//      System.out.println("Zero modulus in xyzCel.");
        ...
        if (xy < 1.0e-11d) {  // on the pole
            ...
            raout = FastMath.atan2(y, x) * Const.HRS_IN_RADIAN;
            decout = FastMath.asin(z) * Const.DEG_IN_RADIAN;
        }
    ...
    double[] precess(final double NewEquinox) {
        ...
        final double cosz = FastMath.cos(z * Const.RADIAN_IN_ARCSEC);
        final double coszeta = FastMath.cos(zeta * Const.RADIAN_IN_ARCSEC);
        final double costheta = FastMath.cos(theta * Const.RADIAN_IN_ARCSEC);
        final double sinz = FastMath.sin(z * Const.RADIAN_IN_ARCSEC);
        final double sinzeta = FastMath.sin(zeta * Const.RADIAN_IN_ARCSEC);
        final double sintheta = FastMath.sin(theta * Const.RADIAN_IN_ARCSEC);
        ...
        final double[] orig = new double[]{
            cosdec * FastMath.cos(alphaRad),
            cosdec * FastMath.sin(alphaRad),
            FastMath.sin(deltaRad)
        };
