...
public final class TargetTransferHandler extends TransferHandler {
    ...
    protected Transferable createTransferable(final JComponent sourceComponent) {
        if (sourceComponent instanceof JList) {
            ...
            final Object[] userObjects = list.getSelectedValues();

            final int size = (userObjects != null) ? userObjects.length : 0;

            if (size != 0) {
                final List<String> targetIds = new ArrayList<String>(size);

                for (int i = 0; i < size; i++) {
                    if (userObjects[i] instanceof Target) {
                        // Target so not null:
                        final Target sourceTarget = (Target) userObjects[i];
                        ...
                        logger.warn("unsupported object type: {}", userObjects[i]);
                    }
                }

                if (!targetIds.isEmpty()) {
                    return new TargetTransferable(targetIds);
                }
            }
    ...
    protected void exportDone(final JComponent sourceComponent, final Transferable data, final int action) {
        if (data != null
            ...
            if (transfer != null) {
                ...
                final Target srcTarget = getTargetById(transfer.getSingleTargetId());
                if (srcTarget != null) {
                    logger.debug("target (calibrator): {}", srcTarget);
