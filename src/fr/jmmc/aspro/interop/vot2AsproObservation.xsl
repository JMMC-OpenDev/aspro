  ...
  -->          
      ...
      <context>
        ...
        <interferometerEditable><xsl:choose>
          <xsl:when test="$CTX_INTERFEROMETER_EDITABLE = 'false'">false</xsl:when><xsl:otherwise>true</xsl:otherwise>
        </xsl:choose></interferometerEditable>
        
        <periodEditable><xsl:choose>
          <xsl:when test="$CTX_PERIOD_EDITABLE = 'false'">false</xsl:when><xsl:otherwise>true</xsl:otherwise>
        </xsl:choose></periodEditable>
        
        <instrumentEditable><xsl:choose>
          <xsl:when test="$CTX_INSTRUMENT_EDITABLE = 'false'">false</xsl:when><xsl:otherwise>true</xsl:otherwise>
        </xsl:choose></instrumentEditable>
        
        <!-- POPS (CHARA only) -->
        <popsEditable><xsl:choose>
          <xsl:when test="$CTX_POPS_EDITABLE = 'false'">false</xsl:when><xsl:otherwise>true</xsl:otherwise>
        </xsl:choose></popsEditable>

        <!-- Configuration(s) -->
        <configurationsEditable><xsl:choose>
          <xsl:when test="$CTX_CONFIGURATIONS_EDITABLE = 'false'">false</xsl:when><xsl:otherwise>true</xsl:otherwise>
        </xsl:choose></configurationsEditable>
        
        <!-- Constraints -->
        <nightEditable><xsl:choose>
          <xsl:when test="$CTX_NIGHT_EDITABLE = 'false'">false</xsl:when><xsl:otherwise>true</xsl:otherwise>
        </xsl:choose></nightEditable>
        
        <dateEditable><xsl:choose>
          <xsl:when test="$CTX_DATE_EDITABLE = 'false'">false</xsl:when><xsl:otherwise>true</xsl:otherwise>
        </xsl:choose></dateEditable>
        
        <minElevationEditable><xsl:choose>
          <xsl:when test="$CTX_MIN_ELEVATION_EDITABLE = 'false'">false</xsl:when><xsl:otherwise>true</xsl:otherwise>
        </xsl:choose></minElevationEditable>

        <!-- TARGETS -->
        <targetsEditable><xsl:choose>
          <xsl:when test="$CTX_TARGETS_EDITABLE = 'false'">false</xsl:when><xsl:otherwise>true</xsl:otherwise>
