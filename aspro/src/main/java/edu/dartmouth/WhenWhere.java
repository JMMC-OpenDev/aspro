...
public final class WhenWhere implements Cloneable {
    ...
    double[] barycenter = null; // LBO: lazily instanciated
    // Barycentric coords in epoch of date; 0-2 are XYZ, 3-5 are velocity.
    double[] altazparSun = null; // LBO: lazily instanciated
    Sun sun;
    HA hasun = null; // LBO: lazily instanciated
    double altsun, azsun;
    double twilight;
    double[] altazparMoon = null; // LBO: lazily instanciated
    Moon moon;    // mostly for use in rise-set calculations.
    HA hamoon = null; // LBO: lazily instanciated
...
//      copy.where = where.clone();
            ...
            copy.barycenter = null;
            copy.altazparSun = null;
            copy.altazparMoon = null;
...
//       System.out.printf(" baryc   %f %f %f%n",p.barycor[0],p.barycor[1],p.barycor[2]);
        ...
        // LBO: lazily instanciate array:
        if (barycenter == null) {
            barycenter = new double[6];
        }
...
//              barycenter[i] / Const.LIGHTSEC_IN_AU, barycenter[i+3]);
        ...
        final double[] geopos = new double[3];
        Topo.Geocent(sidereal, where.lat.value, where.elevsea, geopos);
...
//       System.out.printf("Updated sun: %s %f%n",sun.topopos.checkstring(),altazpar[0]);
    ...
    private void updateSun() {
        if (hasun == null) {
            ...
            hasun.setHA(sidereal - sun.topopos.alpha.value);
        }
        // LBO: lazily instanciate array:
        if (altazparSun == null) {
            altazparSun = new double[3];
        }
...
//          hamoon.roundedHAString(0,":"),altazpar[0]);
    ...
    private void updateMoon() {
        if (hamoon == null) {
            ...
            hamoon.setHA(sidereal - moon.topopos.alpha.value);
        }
        // LBO: lazily instanciate array:
        if (altazparMoon == null) {
            altazparMoon = new double[3];
        }
